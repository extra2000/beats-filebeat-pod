...
| ------- | ---------- | ----- |
| [![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](https://opensource.org/licenses/MIT) | [![semantic-release](https://img.shields.io/badge/%20%20%F0%9F%93%A6%F0%9F%9A%80-semantic--release-e10079.svg)](https://github.com/semantic-release/semantic-release) | [![Build status](https://ci.appveyor.com/api/projects/status/333p4s7qrkuet3hw/branch/master?svg=true)](https://ci.appveyor.com/project/nikAizuddin/beats-filebeat-pod/branch/master) |
...
```
sudo semodule -i docs_beats_filebeat_pod.cil /usr/share/udica/templates/{base_container.cil,net_container.cil}
```
...
Deploy docs:
```
podman run --rm --network no-internet -p 18080:80 -v ./output/html:/usr/local/apache2/htdocs:ro --security-opt label=type:docs_beats_filebeat_pod.process docker.io/library/httpd:2.4
```

To deploy docs using pod, create pod file:
```
cp -v docs-beats-filebeat-pod.yaml{.example,}
```

Then, deploy docs using pod:
```
podman play kube --network no-internet docs-beats-filebeat-pod.yaml
```
